...
describe 'describing a datafile' do
  ...
  subject do
    @wip = submit 'mimi'
    @wip.original_datafiles.find { |df| df['aip-path'] == File.join(AipArchive::SIP_FILES_DIR, 'mimi.pdf') }
  end
  ...
  describe "premis metadata" do
    ...
    it "should have the sip path inplace for the originalName" do
      ...
      obj_id_type.should_not be_nil
      obj_id_type.content.should == File.join(AipArchive::SIP_FILES_DIR, subject['sip-path'])
    end
...
describe 'a datafile with multiple bitstreams' do
  ...
  it "should have multiple bitstreams" do
    wip = submit 'etd'
    df = wip.original_datafiles.find { |df| df['aip-path'] == File.join(AipArchive::SIP_FILES_DIR, 'etd.pdf') }
    df.describe!
