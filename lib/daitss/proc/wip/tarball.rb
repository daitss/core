    ...
    def make_tarball
      ...
      Dir.chdir temp_dir do
        ...
        # link in old xmlres tarballs
      	# first expand the tarballs in a container directory named with tarball basename
      	# e.g  xmlres-0.tar  is expanded to  a directory xmlres-0.
      	# then rearchive into    xmlres-0 direcrtory.
      	# The purpose is to ensure expansions do not overwrite each other.
      	# equivalent of:
      	#  1.  mkdir xmlres-1 
      	#  2.  tar -xf xmlres-1.tar -C xmlres-1
      	cwd  = `pwd`.chomp
        old_xml_res_tarballs.each do |f|
          # check if it's a valid tar file, if it's not, skip it.
          %x{tar -tf #{f}}
          unless $?.exitstatus != 0
